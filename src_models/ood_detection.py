...
def ood_test(args):
    ...
    if args.modality == "single":
        dl, eval_dl = load_single_modal(args, directory = '../processedData/v2', type = "ood")
    elif args.modality == "multi":
        imu_model = SimpleCNN(classnum=args.num_class)
        imu_model.to(device)
        dl, eval_dl = load_multi_modal(args, directory = '../processedData/v2', type = "ood")
    else:
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            if args.modality == "multi":
                y_hat, features_all = imu_model([i.float(), feature])
